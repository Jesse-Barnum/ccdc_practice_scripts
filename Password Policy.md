...
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1\. Navigate to and Update password Policies using the Command line  |  **Ubuntu / Debian / Mint / Raspberry Pi:** <br> 1\. Edit the first password config file: <pre>> sudo nano /etc/pam.d/common-password </pre> and add the following information into the file under the comment that says 'here are the per-package modules...' <pre> password requisite pam_pwquality.so retry=3 minlen=8 difok=3 ucredit=1 lcredit=1 dcredit=1 </pre> *Take a screenshot*. <br><br>  2\. **If the inject requires you to add a password age requirement**, edit the second password policy configuration configuration file: <pre> sudo nano /etc/login.defs </pre> and update the following information into the file under the comment that says "Password aging contorls" <pre> PASS_MAX_DAYS 90 </pre> <pre> PASS_MIN_DAYS 0 </pre> <pre> PASS_WARN_AGE 7 </pre> <pre> PASS_MIN_LEN 8 </pre> *Take a screenshot*. 
**Rocky / Fedora / CentOS / Alpine / BSD(all):** <br> 1\. Edit the first password config file: <pre>> sudo nano /etc/security/pwquality.conf </pre> and add the following information into the file under the comment that says 'Cannot be set to lower value than 6."' <pre> minlen = 8 </pre> <pre> minclass = 4 </pre> *Take a screenshot*. <br><br>  2\. **If the inject requires you to add a password age requirement**, edit the second password policy configuration configuration file: <pre> sudo nano /etc/login.defs </pre> and update the following information into the file under the comment that says "Password aging contorls" <pre> PASS_MAX_DAYS 90 </pre> <pre> PASS_MIN_DAYS 0 </pre> <pre> PASS_WARN_AGE 7 </pre> <pre> PASS_MIN_LEN 8 </pre> *Take a screenshot*. 
| 2\.    |
