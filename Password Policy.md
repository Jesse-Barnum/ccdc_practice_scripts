...
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|**Ubuntu / Debian / Mint / Raspberry Pi:**  |   1\. Edit the first password config file: <pre>> sudo nano /etc/pam.d/common-password </pre> and add the following information into the file under the comment that says 'here are the per-package modules...' <pre> password requisite pam_pwquality.so retry=3 minlen=8 difok=3 ucredit=1 lcredit=1 dcredit=1 </pre> *Take a screenshot* <br> *Save the File*. <br><br>  2\. *If the inject requires you to add a password age requirement*, edit the second password policy configuration configuration file: <pre> sudo nano /etc/login.defs </pre> and update the following information into the file under the comment that says "Password aging contorls" <pre> PASS_MAX_DAYS 90 <br> PASS_MIN_DAYS 0 <br> PASS_WARN_AGE 7 <br> PASS_MIN_LEN 8 </pre> *Take a screenshot* <br> *Save the File*. <br> 
| **Rocky / Fedora / CentOS / Alpine / BSD(all):** |1\. Edit the first password config file: <pre>> sudo nano /etc/security/pwquality.conf </pre> and add the following information into the file under the comment that says 'Cannot be set to lower value than 6."' <pre> minlen = 8 <br> minclass = 4 </pre> *Take a screenshot* <br> *Save the File*. <br><br>  2\. **If the inject requires you to add a password age requirement**, edit the second password policy configuration configuration file: <pre> sudo nano /etc/login.defs </pre> and update the following information into the file under the comment that says "Password aging contorls" <pre> PASS_MAX_DAYS 90 <br> PASS_MIN_DAYS 0 <br> PASS_WARN_AGE 7 <br> PASS_MIN_LEN 8 </pre> *Take a screenshot* <br> *Save the File*. 
...
After implementing the Password Policy, use the inject report in **public-ccdc-resources/Injects** entitled **secure_password_guidelines.txt/.docx** to complete the inject report, providing the necessary proof of all devices that the password policy is updated. For Windows devices, you will only need to include the screenshot from the Domain Controller. 
